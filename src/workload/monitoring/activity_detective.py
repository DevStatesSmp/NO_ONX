...
def check_user_info():
    ...
    try:
        ...
        print(f"{Fore.YELLOW}[WARN] Error fetching user data: {e.output.decode('utf-8', errors='ignore')}")
    except PermissionError:
        print(f"{Fore.RED}[ERROR] No permission to run 'net user'. Run as administrator.")
...
def monitor_user_activity():
    ...
    try:
        ...
        print(f"{Fore.RED}[ERROR] Unable to open event log: {e}")
        if "access is denied" in str(e).lower():
            print(f"{Fore.YELLOW}[WARN] No permission to read the Security log. Run as administrator.")
        return
    ...
    try:
        ...
        print(f"{Fore.RED}[ERROR] Error reading event log: {e}")
    finally:
        try:
            win32evtlog.CloseEventLog(hand)
            print(f"{Fore.GREEN}[INFO] Log handle closed.")
        except Exception:
            print(f"{Fore.RED}[ERROR] Failed to close log handle.")
